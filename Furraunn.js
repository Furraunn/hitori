						...
						await naze.groupParticipantsUpdate(m.chat, [numbersOnly], 'add').then(async (res) => {
							for (let i of res) {
								...
								if (statusMessages[i.status]) {
									...
									await m.reply(`${'https://chat.whatsapp.com/' + invv}\n------------------------------------------------------\n\nAdmin: @${m.sender.split('@')[0]}\nMengundang anda ke group ini\nSilahkan masuk jika berkehendakðŸ™‡`, { detectLink: true, chat: numbersOnly, quoted: fkontak }).catch((err) => m.reply('Gagal Mengirim Undangan!'))
			...
			case 'runtime': case 'tes': case 'bot': {
				switch(args[0]) {
					...
					let settingsBot = Object.entries(set).map(([key, value]) => {
						let list = key == 'status' ? new Date(value).toLocaleString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : (typeof value === 'boolean') ? (value ? 'onðŸŸ¢' : 'offðŸ”´') : value;
			...
			case 'play': case 'ytplay': case 'yts': case 'ytsearch': case 'youtubesearch': {
				...
				try {
					...
					try {
						...
						let teksnya = `*ðŸ“Title:* ${hasil.title || 'Tidak tersedia'}\n*âœUpload At:* ${hasil.uploaded || 'Tidak tersedia'}\n*ðŸŒŸChannel:* ${hasil.author || 'Tidak tersedia'}\n*â³Duration:* ${hasil.duration || 'Tidak tersedia'}\n*ðŸ”ŽSource:* ${hasil.url || 'Tidak tersedia'}\n\n_note : jika ingin mendownload silahkan_\n_pilih ${prefix}ytmp3 url_video atau ${prefix}ytmp4 url_video_`;
						await m.reply({ image: { url: hasil.thumbnail }, caption: teksnya })
					} catch (e) {
						try {
							const res = await fetchApi('/search/youtube', { query: text });
							const hasil = pickRandom(res.data)
							const teksnya = `*ðŸ“Title:* ${hasil.title || 'Tidak tersedia'}\n*âœDescription:* ${hasil.description || 'Tidak tersedia'}\n*ðŸŒŸChannel:* ${hasil.channelTitle || 'Tidak tersedia'}\n*â³Duration:* ${hasil.duration || 'Tidak tersedia'}\n*ðŸ”ŽSource:* https://youtu.be/${hasil.id || 'Tidak tersedia'}\n\n_note : jika ingin mendownload silahkan_\n_pilih ${prefix}ytmp3 url_video atau ${prefix}ytmp4 url_video_`;
			...
			case 'fb': case 'fbdl': case 'fbdown': case 'facebook': case 'facebookdl': case 'facebookdown': case 'fbdownload': case 'fbmp4': case 'fbvideo': {
				...
				try {
					...
					if (hasil.results.length < 1) {
						...
						m.reply(mess.wait)
						await naze.sendFileUrl(m.chat, hasil.results[0].url, `*ðŸŽTitle:* ${hasil.caption}`, m);
					}
					setLimit(m, db)
				} catch (e) {
